      ...
      if (client.available()) {
        ...
        if(pin != NULL){
          if(value != NULL){
            ...
            if(strncmp(value, "HIGH", 4) == 0 || strncmp(value, "LOW", 3) == 0){
            ...
            else if (strncmp(pin, "GARAGE", 6) == 0)  {
            
              Serial.println("wooo garage!");
              ...
              if(strncmp(value, "OPEN", 4) == 0){
                ...
                // Only do something if the garage is closed
                
                if (switchValue == 1) {
                  
                  // Switch is high so the garage is closed
                  // Open it!
                  ...
                  client.println("HTTP/1.1 200 OK");
                  client.println("Content-Type: text/html");
                  client.println("GARAGE IS ALREADY OPEN");
                }
              } else if(strncmp(value, "CLOSE", 4) == 0) {
                //  Closing Garage
                ...
                // Only do something if the garage is open
                
                if (switchValue == 0) {
                  // Switch is low so the garage is open
                  // Close it!
                  int selectedPin = 7;
